...
const ExploreCourses = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return courses.filter((c) => {
      ...
      const searchMatch =
        !q ||
        [c.title, c.instructor, c.category].some((v) =>
    ...
    <div>
      ...
      <div className="card p-3 mb-4">
        ...
        {showFilters && (
          <div className="mt-3 pt-3 border-top">
            <div className="small text-muted mb-2">Filter by category</div>
            <div className="d-flex gap-2 flex-wrap">
              {categories.map((cat) => (
                <button
                  key={cat}
                  type="button"
                  className={`btn btn-sm ${activeCat === cat ? "btn-primary" : "btn-outline-secondary"} rounded-pill`}
      ...
      {filtered.length === 0 ? (
        <div className="card p-5 text-center">
          <FaSearch className="text-muted mb-3" size={28} />
          <h6 className="fw-bold">No courses found</h6>
